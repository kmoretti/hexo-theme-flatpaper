...
- `text` 是可选项，对应 `fonts` 的 Google Fonts `text=` 参数。仅建议在能确定字符范围时使用，例如 logo、标题、按钮等固定文本；未包含的字符会回退到后续字体。
- `mono` 会单独请求完整字体，不受 `text` 限制，避免代码字符缺失。
...
- `image_overlay` 控制图片开屏的暗色遮罩，使用 `[顶部, 底部]` 两个 `0` 到 `1` 的透明度数值；默认是 `[0.2, 0.2]`。
