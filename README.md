...
启用 Twikoo 后，评论区会在**每个页面的正文之后**渲染（文章页位于"相关文章"卡片下方）。需要自行部署 Twikoo 后端，参考 [Twikoo 快速开始](https://twikoo.js.org/quick-start.html)。不想要评论的页面（友链页、关于页、tag/分类页等）可以在 front-matter 写 `comments: false` 关闭。文章页 reactions 行的"评论"图标按钮会平滑滚动到 `#tcomment`，分享按钮调用 `navigator.share`（不支持时复制 URL 到剪贴板）。
